...
@mcp_server.tool(
    ...
    description="""
    获取同花顺问财平台的概念板块资金流向数据，显示各热门概念的资金净流入/流出情况。

    数据包含字段：
    - 序号：概念板块排名顺序
    - 行业：具体概念板块名称（如CRO概念、创新药、减肥药、细胞免疫治疗等）
    - 公司家数：该概念板块包含的相关上市公司数量
    - 行业指数：对应的概念板块指数数值
    - 阶段涨跌幅：指定时间周期内的涨跌幅度，显示为百分比格式（如16.82%）
    - 流入资金：资金流入金额（亿元）
    - 流出资金：资金流出金额（亿元）
    - 净额：净流入金额，正数表示净流入，负数表示净流出（亿元）
    ...
    注意事项：
    - 概念板块数据波动性较大，建议结合多个时间周期分析
    - 净额为负值表示该概念板块资金净流出
    - 建议关注净流入较大且涨跌幅适中的概念板块
    """
)
